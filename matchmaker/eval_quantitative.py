...
def evaluate_model(model, config, _logger, cuda_device, eval_tsv, use_cache=False,
                   output_secondary_output: bool = False, save_embeddings=True):
    ...
    try:
        ...
        if use_submodel_caching:
            submodel_cacher = CrossExperimentReplayCache(
                os.path.join(config["submodel_validation_cache_path"], os.path.basename(os.path.dirname(eval_tsv))),
                is_readonly=config["submodel_validation_cache_readonly"])
        ...
        if save_embeddings:
            all_document_embeddings = defaultdict(lambda: [[] for _ in range(len(model.neural_ir_model.contextualizer.layers))])
            all_query_embeddings = defaultdict(lambda: [[] for _ in range(len(model.neural_ir_model.contextualizer.layers))])
            # print([[] for _ in range(len(model.neural_ir_model.contextualizer.layers))])
            # print(all_query_embeddings["something"])
        with torch.no_grad():
            pair_count = 0
            if train_sparsity:
                stopwords = 0
                total_words = 0
            i = 0
            for batch_orig in tqdm(input_loader):
                with torch.cuda.amp.autocast(enabled=config["use_fp16"]):
                    ...
                    if save_embeddings:
                        query_embeddings, document_embeddings = model.forward_embeddings_only(*args_in,
                                                                                              output_secondary_output=output_secondary_output,
                                                                                              use_fp16=use_fp16)
                        for layer in range(1, len(document_embeddings)):
                            for batch_index in range(len(document_embeddings[layer])):
                                for token_index in range(len(document_embeddings[layer][batch_index])):
                                    # if document_embeddings[0][batch_index, token_index, :].sum() != 0:
                                    embedding_list = all_document_embeddings[args_in[1]["tokens"]["tokens"][batch_index, token_index].item()][layer - 1]
                                    if len(embedding_list) > num_samples:
                                        index = torch.randint(0, len(embedding_list), (1,))
                                        embedding_list[index] = document_embeddings[layer][batch_index, token_index, :]
                                    else:
                                        embedding_list.append(document_embeddings[layer][batch_index, token_index, :])

                                for token_index in range(len(query_embeddings[layer][batch_index])):
                                    # if query_embeddings[0][batch_index, token_index, :].sum() != 0:
                                    embedding_list = all_query_embeddings[args_in[1]["tokens"]["tokens"][batch_index, token_index].item()][layer - 1]

                                    if len(embedding_list) > num_samples:
                                        index = torch.randint(0, len(embedding_list), (1,))
                                        embedding_list[index] = query_embeddings[layer][batch_index, token_index, :]
                                    else:
                                        embedding_list.append(query_embeddings[layer][batch_index, token_index, :])
                    ...
                    for sample_i, sample_query_id in enumerate(batch_orig["query_id"]):  # operate on cpu memory
                        ...
                        if train_qa_spans:
                            if batch_orig["qa_hasAnswer"][sample_i] == 1:
                                qa_validation_results_stats["QA/Start_accuracy"].append(
                                    1 if start_idx[sample_i] in batch_orig["qa_start"][sample_i] else 0)
                                qa_validation_results_stats["QA/End_accuracy"].append(
                                    1 if end_idx[sample_i] in batch_orig["qa_end"][sample_i] else 0)
                                qa_validation_results_stats["QA/Full_accuracy"].append(
                                    1 if end_idx[sample_i] in batch_orig["qa_end"][sample_i] and start_idx[sample_i] in
                                         batch_orig["qa_start"][sample_i] else 0)

                            qa_validation_results_stats["QA/Answerability_accuracy"].append(
                                float(answerable[sample_i] == batch_orig["qa_hasAnswer"][sample_i]))
                            if answerable[sample_i]:
                                ...
                                if oe != None and os != None and oe >= os:
                                    qa_validation_results[sample_query_id][sample_doc_id] = (
                                    bool(answerable[sample_i]), int(start_idx[sample_i]), int(end_idx[sample_i]),
                                    batch_orig["doc_text"][sample_i][os:oe])
                                else:
            ...
            # _logger.info("val_close") dfa

            if save_embeddings:
                # with open('company_data.pkl', 'wb') as outp:

                current_embedding_id = 0
                for all_embedding in [all_query_embeddings, all_document_embeddings]:
                    embedding_averages = [0] * len(model.neural_ir_model.contextualizer.layers)
                    # print("all_embedding", all_embedding)
                    count = [0] * len(model.neural_ir_model.contextualizer.layers)
                    for input_id, embeddings_per_layer in all_embedding.items():
                        for (i, layer) in enumerate(embeddings_per_layer):
                            indices = range(len(layer))

                            if len(layer) <= 1:
                                break

                            if (len(layer)**2) > num_samples:
                                indices_1 = torch.randint(0, (len(layer)), (num_samples*2,))
                                indices_2 = torch.randint(0, (len(layer)), (num_samples*2,))

                                index_pairs = [(i.item(), j.item()) for i, j in zip(indices_1, indices_2) if i != j]
                                if len(index_pairs) > num_samples:
                                    index_pairs = index_pairs[:num_samples]
                            else:
                            ...
                            for ii, jj in index_pairs:
                                layer_similarities.append(1 - torch.cosine_similarity(layer[ii], layer[jj], dim=0))

                            layer_similarities_stacked = torch.stack(layer_similarities)

                            mean_layer_similarity = torch.mean(layer_similarities_stacked)
                            embedding_averages[i] += mean_layer_similarity
                            count[i] += 1

                    for i, score in enumerate(embedding_averages):
                        embedding_averages[i] = score / count[i]
                    current_embedding_id += 1
                    print("averages", embedding_averages)
...
def calculate_inter_context(layer):

    matrix = torch.stack(layer)
    print(matrix.size())
    normalized_matrix = matrix / matrix.norm(dim=1)[:, None]
    cosine_simularity_matrix = torch.mm(normalized_matrix, normalized_matrix.transpose(0, 1))
    return torch.mean(cosine_simularity_matrix, dim=(0, 1))
...
def validate_model(val_name, model, config, validation_config, run_folder, _logger, cuda_device,
    ...
    validation_results, qa_validation_results, qa_validation_results_stats, pair_count, secondary_output = evaluate_model(
        model, config, _logger, cuda_device, validation_config["tsv"], use_cache, output_secondary_output)
    ...
    if output_secondary_output:
        ...
        if is_distributed:
            m = model.module
        save_secondary_output(m, os.path.join(run_folder, "secondary-" + evaluation_id + ".npz"), ranked_results,
                              secondary_output, config["secondary_output"]["top_n"])
    ...
    if candidate_set != None:
        ...
        metrics = calculate_metrics_along_candidate_depth(ranked_results, load_qrels(validation_config["qrels"]),
                                                          candidate_set, validation_config["candidate_set_from_to"],
                                                          validation_config["binarization_point"])
        ...
        save_one_metric_multiN(metric_file_path, metrics,
                               config["validation_metric"], range(validation_config["candidate_set_from_to"][0],
                                                                  validation_config["candidate_set_from_to"][1] + 1),
                               epoch_number, batch_number)

        # save all info + get best value of validation metric
        best_metric_value = 0
        for current_cs_n in range(validation_config["candidate_set_from_to"][0],
                                  validation_config["candidate_set_from_to"][1] + 1):
        ...
        if val_name != "cont":
            save_fullmetrics_rangeN(os.path.join(run_folder, "val-" + evaluation_id + "-all-metrics.csv"), metrics,
                                    range(validation_config["candidate_set_from_to"][0],
                                          validation_config["candidate_set_from_to"][1] + 1))
...
#
def test_model(model, config, test_config, run_folder, _logger, cuda_device, test_name, candidate_set=None,
               candidate_set_n=None, output_secondary_output=False, is_distributed=False):
    test_results, qa_validation_results, qa_validation_results_stats, _, secondary_output = evaluate_model(model,
                                                                                                           config,
                                                                                                           _logger,
                                                                                                           cuda_device,
                                                                                                           test_config[
    ...
    if "qrels" in test_config:
        ...
        if candidate_set != None:
            metrics = calculate_metrics_single_candidate_threshold(ranked_results, load_qrels(test_config["qrels"]),
                                                                   candidate_set, candidate_set_n,
                                                                   test_config["binarization_point"])
    ...
    if output_secondary_output:
        ...
        if is_distributed:
            m = model.module
        save_secondary_output(m, os.path.join(run_folder, "secondary-" + test_name + ".npz"), ranked_results,
                              secondary_output, config["secondary_output"]["top_n"])
...
def save_secondary_output(model, out_file, ranked_results, secondary_output, max_sec_i):
    ...
    numpy.savez_compressed(out_file, model_data=model_data_secondary_cpu, qd_data=filtered_secondary_output,
                           per_batch_info=secondary_output["per_batch_info"])
...
def save_fullmetrics_oneN(file, metrics, epoch_number, batch_number):
    ...
    with open(file, "a") as metric_file:
        metric_file.write(
            str(epoch_number) + "," + str(batch_number) + "," + ",".join(str(v) for k, v in metrics.items()) + "\n")
...
def save_best_info(file, best_info):
    with open(file, "w") as metric_file:
        ...
        metric_file.write(str(best_info["epoch"]) + "," + str(best_info["batch_number"]) + "," + str(
            best_info["cs@n"]) + "," + ",".join(str(v) for k, v in best_info["metrics"].items()) + "\n")
...
def save_one_metric_multiN(file, metrics, selected_metric, over_range, epoch_number, batch_number):
    ...
    # append single row
    with open(file, "a") as metric_file:
        metric_file.write(str(epoch_number) + "," + str(batch_number) + "," + ",".join(
            str(v[selected_metric]) for cs_at_n, v in metrics.items()) + "\n")

# def save_contextualization(file, contextualization):
#     with open(file, "w") as metric_file:
#         metric_file.write("sep=,\nEpoch,batch_number,cs@n," + ",".join(k for k, v in best_info["metrics"].items()) + "\n")
#         metric_file.write()
#         # metric_file.write(str(best_info["epoch"]) + "," + str(best_info["batch_number"]) + "," + str(
